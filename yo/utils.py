...
def needs_auth(func):
    ...
    def func_wrapper(*args, **kwargs):
        ...
        if not 'username' in kwargs.keys():
            return {
                'error':
                'Could not authenticate request, missing username parameter',
                'status':
                'error',
                'error_type':
                'MISSING_ARGS'
        ...
        if not 'orig_req' in kwargs.keys():
            return {
                'error': 'Could not authenticate request, internal error',
                'status': 'error',
                'error_type': 'INTERNAL'
            }  # we don't want to leak info here
        if not (type(kwargs['orig_req']) is dict):
            return {
                'error':
                'Could not authenticate request, request is malformed, must be a dictionary',
                'status':
                'error',
                'error_type':
                'MALFORMED_REQUEST'
        ...
        if not 'params' in kwargs['orig_req'].keys():
            return {
                'error':
                'Could not authenticate request, request is malformed, lacking params',
                'status':
                'error',
                'error_type':
                'MALFORMED_REQUEST'
