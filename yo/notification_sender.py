...
class YoNotificationSender(YoBaseService):
    ...
    async def run_send_notify(self, notification_job):
        ...
        for transport_name, transport_data in user_transports.items():
            for notify_type in transport_data['notification_types']:
                ...
                user_notify_types_transports[notify_type].append((transport_name, transport_data['sub_data']))


        with self.db.acquire_conn() as conn:
            query = notifications_table.select().where(
                    notifications_table.c.to_username == notification_job['to_username'])
            query = query.where(notifications_table.c.sent_at is None)
    ...
    def init_api(self, yo_app):
        ...
        if yo_app.config.config_data['sendgrid'].get('enabled', False) is True:
        ...
        if yo_app.config.config_data['twilio'].get('enabled', False) is True:
