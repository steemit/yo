...
wwwpoll_table = sa.Table(
        ...
        sa.Column('shown', sa.Boolean(), nullable=True, default=False),
        sa.Column('username', sa.String(20), index=True),
        sa.Column('data', sa.UnicodeText),
        sa.UniqueConstraint('username','notify_type','data'),
        mysql_engine='InnoDB',
)
...
notifications_table = sa.Table(
        ...
        sa.Column('sent', sa.Boolean(), nullable=True, default=False,
                  index=True),
        ...
        sa.UniqueConstraint('to_username','type','trx_id','from_username','json_data',name='yo_notifcations_idx'),
        mysql_engine='InnoDB',
)
...
class YoDatabase:
    def __init__(self, config, initdata=None):
        ...
        if config.config_data['database'].get('reset_db', False) is True:
            logger.info('Wiping old data due to YO_DATABASE_RESET_DB=True')
            metadata.drop_all(self.engine)
            logger.info('Finished wiping old data')

        if config.config_data['database'].get('init_schema', False) is True:
        ...
        if initdata is None:
            ...
            if initdata_file is None:
                ...
                with open(initdata_file, 'rb') as f:
    ...
    def start_tx(self):
        ...
        try:
            ...
            logger.exception('start_tx failed')
            tx.rollback()
        finally:
            conn.close()

    @contextmanager
    def select_notification_for_update(self, notification):
        try:
            tbl = notifications_table
            with self.acquire_conn as conn:
                with self.start_tx() as tx:
                    stmt = tbl.select().with_for_update(read=False)
                    stmt = stmt.where(tbl.c.to_username == notification['to_username'])
                    stmt = stmt.where(tbl.c.from_username == notification['from_username'])
                    stmt = stmt.where(tbl.c.json_data == notification['json_data'])
                    stmt = stmt.where(tbl.c.type == notification['type'])
                    if notification['trx_id']:
                        stmt = stmt.where(tbl.c.trx_id == notification['trx_id'])
                    result = conn.execute(stmt).first()
                    yield result
        except:
            logger.exception('select_notification_for_update failed')
        finally:
            pass
